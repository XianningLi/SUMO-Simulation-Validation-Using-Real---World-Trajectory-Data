...
global updateIndx, DxxS, IxxS, IxuS, P, u, change_gain, accelFT, if_des_lane, init1, initateLaneChange, initateLaneAbort, randNum, e20, agg_gain_time2, ttc_AV_FT_all, ttc_LT_AV_all, ttc_LC_AV_all, St, lat_acc_AV, long_acc_AV
...
def generate_routefile():
     ...
     with open("NGSIMTest.rou.xml", "w") as routes: 
         ...
         print("""<routes>
         <vType id="auto" accel="3.8" decel="3.8" length="4.4" width="1.5" minGap="1" maxSpeed="15" guiShape="passenger" color="1,0,0"/>
         <vType id="human" accel="3.8" decel="3.8"  length="4.4" width="1.5" minGap="1" maxSpeed="15" guiShape="passenger" color="0,0,1"/>
         <route id="right" edges="AB"/>""", file=routes)
           
         print('<vehicle id="AV" type="auto" route="right" depart="0" departSpeed="4.572" departLane="0" departPos="51.5285736" />', file=routes) # AV (s)
         print('<vehicle id="FC" type="human" route="right" depart="0" departSpeed="3.599688" departLane="0" departPos="35.4049584" />', file=routes) # FC (r) (1)
         print('<vehicle id="FT" type="human" route="right" depart="0" departSpeed="3.599688" departLane="1" departPos="21.8498928" />', file=routes) # FT (rt) (2)
         ...
         print('<vehicle id="LT" type="human" route="right" depart="0" departSpeed="4.559808" departLane="1" departPos="58.1765664" />', file=routes) # LT (ft) (4)
...
def ChooseGain(vel_AV):
         ...
         K10p3L = np.array([[4.3626,    1.1417,   94.5364,   53.86092]])
         K11p1L = np.array([[4.2204,    1.1042,   99.3193,   53.9053]])
         K11p9L = np.array([[4.5792,    1.1967,  104.6983,   53.8391]])
         K12p7L = np.array([[4.0107,    1.1984,  106.2317,   53.8281]])
         K13p5L = np.array([[4.3561,    1.2548,  113.0002,   53.7966]])
         K14p3L = np.array([[4.5240,    1.2396,  118.6122,   53.8280]])
         K15p1L = np.array([[4.4458,    1.2388,  123.2048,   53.8168]])
         ...
         if (vel_AV>9.5) and (vel_AV<=10.3):
                   print("vel_AV", vel_AV, ", in: 9.5 to 10.3")
                   K = K9p5L + (-K9p5L+K10p3L)*( (vel_AV-0.5)/((space)) )
                   # K = K9p5L
                  
         if (vel_AV>10.3) and (vel_AV<=11.1):
                    print("vel_AV", vel_AV, ", in: 10.3 to 11.1")
                    K = K10p3L + (-K10p3L+K11p1L)*( (vel_AV-10.3)/((space)) )
                    # K = K10p3L 
                    
         if (vel_AV>11.1) and (vel_AV<=11.9):
                    print("vel_AV", vel_AV, ", in: 11.1 to 11.9")
                    K = K11p1L + (-K11p1L+K11p9L)*( (vel_AV-11.1)/((space)) )
                    # K = K11p1L
                    
         if (vel_AV>11.9) and (vel_AV<=12.7):
                    print("vel_AV", vel_AV, ", in: 11.9 to 12.7")
                    K = K11p9L + (-K11p9L+K12p7L)*( (vel_AV-11.9)/((space)) )
                    # K = K11p9L 
                    
         if (vel_AV>12.7) and (vel_AV<=13.5):
                    print("vel_AV", vel_AV, ", in: 12.7 to 13.5")
                    K = K12p7L + (-K12p7L+K13p5L)*( (vel_AV-12.7)/((space)) )
                    # K = K12p7L 

                    
         if (vel_AV>13.5) and (vel_AV<=14.3):
                   print("vel_AV", vel_AV, ", in: 13.5 to 14.3")
                   K = K13p5L + (-K13p5L+K13p5L)*( (vel_AV-13.5)/((space)) )
                   # K = K13p5L 
                   
         if (vel_AV>14.3) and (vel_AV<=15.1):
                   print("vel_AV", vel_AV, ", in: 14.3 to 15.1")
                   K = K14p3L + (-K15p1L+K14p3L)*( (vel_AV-14.3)/((space)) )
                   # K = K14p3L
                    
         if (vel_AV>15.1):
             print("vel_AV", vel_AV, ", in: > 15.1")
             K = K15p1L
...
def run(h=0.65, AV_constant_vel=0):
     ...
     while (step < N1):        
         ...
         global DxxS, IxxS, IxuS, P, u, change_gain, accelFT, if_des_lane, initateLaneChange, initateLaneAbort, randNum, e20, agg_gain_time2, ttc_AV_FT_all, ttc_LT_AV_all, ttc_LC_AV_all, St, lat_acc_AV, long_acc_AV
         ...
         if traci.simulation.getTime() > 10:
             ...
             r = np.array([Leader_x-(h*vel_LT+distStand+carW+4.4), Leader_vel])
             # Kl = np.array([[4.47213595499958,	110.382108140762]])
             Kl = np.array([[10.0000000003215,	165.227116422160]])
             if OutReg == 1:
                 # Kl = np.array([[0.0715262765604084,	0.384583524450610]])
                 # Ll = np.array([[0.0714426181991808,	0.390262863029821]])
                 Kl = np.array([[0.316235341317086,	0.855854112990615]])
             ...
             if (traci.vehicle.getLaneID("AV") == "AB_1"):
                 if_des_lane = 1
             ...
             print("AV Alne ID: ", traci.vehicle.getLaneID("AV"))
             print("LeaderID: ", LeaderID, "safety from FT: ", (Pos_AV_x >= Pos_FT_x+(h*vel_FT+distStand+carW)), "safety from LT: ", (Pos_AV_x <= Pos_LT_x-(h*vel_LT+distStand+carW)))
             ...
             print("cos(theta): ", np.cos(math.radians(ang-90)))
             # ---- choosing the target point in y ----- #   
             if weight1 <= 0:
                 weight1 = 0
             if weight2 >= 1:
                 weight2 = 1             
             targetY = weight1*Leader_y + weight2*5.7
             weight1 = weight1-0.001
             weight2 = weight2 + 0.001
             # ----------------------------------------- #
                 
             ## ----------------------- Move the AV --------------------------------- ##
             traci.vehicle.moveToXY(vehID='AV',edgeID='AB', lane=1, x=Xk_AV0[0], y=-e1+targetY, angle=ang, matchThreshold=100000) #5.7 is the center of lane 1
         ...
         AV_lat_vel = traci.vehicle.getLateralSpeed('LT')
         
         print("LT_lat_vel: ", AV_lat_vel)
         
         
         safe_dist = np.append(safe_dist, np.array([[h*vel_LT+distStand+carW],[h*vel_FT+distStand+carW],[h*vel_LC+distStand+carW],[h*vel_FC+distStand+carW]]), axis=-1)
         lat_acc_AV = np.append(lat_acc_AV, np.array([[accel_Auto*np.sin(np.pi/2-angle_AV)]]), axis=-1)
         long_acc_AV = np.append(long_acc_AV, np.array([[accel_Auto*np.cos(np.pi/2-angle_AV)]]), axis=-1)
         time_all = np.append(time_all,np.array([[time]]))
     ...
     savemat("compare_NGSIM_DS5_"+str(h)+".mat", {"Xk_AV":Xk_AV,"e":e, "y_ang":y_ang, "lat_acc_AV": lat_acc_AV, "long_acc_AV": long_acc_AV,\
                                                 ...
                                                 "safe_dist":safe_dist,  "pos_y_all":pos_y_all, "steer_angle_all": steer_angle_all})
